"""
=============================================================================
Citrus Disease Detection — ResNet50 (Proposed Model)
=============================================================================
Binary classification: Citrus Canker (CC) vs Black Spot
Designed to run on Google Colab with GPU runtime.

Expected dataset structure (after upload/unzip in Colab):
...
# IMPORTANT: For ResNet50, BatchNorm layers must be kept in inference mode
# to avoid disturbing pretrained statistics. This is the standard fix that
# allows ResNet50 to converge properly.
# ---------------------------------------------------------------------------
print("\n" + "="*70)
print("PHASE 2: Fine-tuning top layers (BatchNorm kept frozen)")
print("="*70)
base_model.trainable = True
# Freeze all but the last ~30 layers
for layer in base_model.layers[:-30]:
    layer.trainable = False
# Keep ALL BatchNorm layers frozen — critical for ResNet50
...
axes[0].plot(acc_hist, label="Train"); axes[0].plot(val_acc, label="Val")
axes[0].set_title(f"{MODEL_NAME} Accuracy"); axes[0].legend(); axes[0].grid(True)
axes[1].plot(loss_hist, label="Train"); axes[1].plot(val_loss, label="Val")
axes[1].set_title(f"{MODEL_NAME} Loss"); axes[1].legend(); axes[1].grid(True)
plt.tight_layout()
plt.savefig(f"/content/curves_{MODEL_NAME}.png", dpi=150)
plt.show()
